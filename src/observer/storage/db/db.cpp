...
RC Db::drop_table(const char *table_name) {
  ...
  //VECTORS类型存放数据路径
  auto table_vector_name = table_vector_file(path_.c_str(), table_name);
  if(unlink(table_vector_name.c_str()) == -1){
    // 删除元文件数据
    LOG_ERROR("failed to delete table (%s) text file %s.",table_name,table_vector_name.c_str());
  }
