    ...
    auto cmp = [this](const auto &a, const auto &b) {
        for (size_t i = 0; i < order_by_expressions_.size(); ++i) {
            ...
            if (cell_a.is_null() && !cell_b.is_null()) {
                 return static_cast<bool>(order_by_directions_[i]); // cell_a 是 null，cell_b 不是，cell_a 排在前面
            }

            if (!cell_a.is_null() && cell_b.is_null()) {
               return !static_cast<bool>(order_by_directions_[i]); // cell_b 是 null，cell_a 不是，cell_a 排在后面
            }

            int comparison = cell_a.compare(cell_b); // 使用 compare 函数进行比较
            if (comparison != 0) {
                return static_cast<bool>(order_by_directions_[i]) ? (comparison < 0) : (comparison > 0); // 根据排序方向返回
            }
        }
        return false; // 完全相同
    };
