...
public:
  CreateTableStmt(
    Db *db,
      ...
      : db_(db),table_name_(table_name), attr_infos_(attr_infos), storage_format_(storage_format),select_stmt_(select_stmt)
  {
    // 插入null
  ...
  static void populateAttrInfo(const std::unique_ptr<Expression> &attr_expr, AttrInfoSqlNode &attr_info);
private:
  Db *db_=nullptr;
