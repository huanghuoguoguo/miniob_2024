...
RC UpdateStmt::create(Db *db, const UpdateSqlNode &update_sql, Stmt *&stmt)
{
  ...
  // 验证字段(属性)名以及Value
  const std::string &field_name = update_sql.attribute_name;
  ...
  // 2. 创建一个未绑定的比较表达式，用于表示set条件中的 field_name = value (name=1)
  std::unique_ptr<Expression> unbound_comp_expr = std::make_unique<ComparisonExpr>(EQUAL_TO,
