...
RC LogicalPlanGenerator::create_plan(SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  for (Table *table : tables) {
    ...
    if (table_oper == nullptr) {
      ...
      for(const auto& join_filter:join_filter_stmts) {
        if(std::get<0>(join_filter) == table || std::get<1>(join_filter) == table) {
          // 存在该table的join条件。
          unique_ptr<LogicalOperator> predicate_oper(nullptr);
          RC rc = create_plan(std::get<2>(join_filter), predicate_oper);
          if (OB_FAIL(rc)) {
